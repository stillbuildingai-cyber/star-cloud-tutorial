...
> ℹ️ 登入後你會發現側邊選單只有「機台管理」「分析管理（機台報表）」「資料設定（子帳號）」「遠端管理（指令中心）」
> 「基本設定（機台設定）」「權限設定」這幾項——這套後台原本是給商用販賣機隊用的通用平台，功能遠不只這些。
> 教學版把跟智慧插座無關的功能（倉庫、銷售、商品、廣告、APK 版本、金流設定、Discord 通知等）連同對應的
> Controller、頁面、路由都整支移除了，不是單純用權限隱藏，所以在「權限設定」裡也不會找到能把它們打開的選項。
> 如果你想學完整的商用機隊管理架構（含這些功能），可以參考正式商用版本，這不在本教學系列的範圍內。
